      ...
      achievementDataArray.map(async (achievementData: any) => {
        if (achievementData._id) {
          ...
          if (existingAchievement) {
            return await Achievement.findByIdAndUpdate(achievementData._id, achievementData, {
              new: true,
            });
          }
        }
...
export async function addCertificateToResume(
  ...
  try {
      ...
      certificateDataArray.map(async (certificateData: any) => {
        if (certificateData._id) {
          ...
          if (existingCertificate) {
            return await Certificate.findByIdAndUpdate(certificateData._id, certificateData, {
              new: true,
            });
          }
        }
    ...
    const certificateIds = savedCertificates.map((certificate) => certificate._id);
