...
import { generateCertificateDescription } from "@/lib/actions/gemini.actions";
...
const CertificateForm = ({ params }: { params: { id: string } }) => {
  ...
  const RemoveCertificate = () => {
    const newEntries = certificateList.slice(0, -1);
    setCertificateList(newEntries);
  ...
  const generateDescriptionFromAI = async (index: number) => {
    if (
      !formData?.certificates?.[index]?.title ||
      formData?.certificates?.[index]?.title === ""
    ) {
      toast({
        title: "Uh Oh! Something went wrong.",
        description:
          "Please enter the certificate title to generate a description.",
        variant: "destructive",
  ...
  const onSave = async (e: any) => {
    ...
    const result = await addCertificateToResume(params.id, formData.certificates);
    ...
    <div>
      <div className="p-5 shadow-lg rounded-lg border-t-primary-700 border-t-4 bg-white">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Add any notable certificates or licenses.
        </p>

        <div className="mt-5">
          {certificateList.map((item: any, index: number) => (
            <div key={index}>
